...
liczba_ludn<-readxl::read_xlsx('~/ADP2020/personal/olamilczarek/data/liczba_ludn.xlsx', sheet = 2)
...
liczba_ludn <- liczba_ludn[-c(1, 5), ]
names(liczba_ludn)[names(liczba_ludn) == "Kod"] <- "JPT_KOD_JE"
...
df <- powiaty %>%
  ...
  left_join(liczba_ludn,by=c('JPT_KOD_JE'))
...
ggplot(df,aes(x =`ludność na 1 km2` , y= liczba_zachorowan)) + 
  geom_point(size = 3) + 
...
ggplot(df,aes(x = wspol_wiek, y= liczba_zachorowan)) + 
  geom_point(size = 3) + 
  geom_smooth(method = lm) +
  scale_x_log10()+
  #scale_y_log10()+
  labs(title = "Korelacja między liczba przypadkow, a ilościa ludzi powyzej 70lat", 
       y= "liczba zachorowań", 
       x = "współczynnik osób powyżej 70 roku życia")
...
ggplot(df,aes(x =srednia_zarobki , y=liczba_zachorowan)) + 
  geom_point(size = 3) + 
  geom_smooth(method = lm) +
  #scale_x_log10()+
  scale_y_log10()+
...
df$wsp_100<-(df$liczba_zachorowan/df$liczba_ludn)*100000

ggplot()+ 
  geom_sf(data = df,aes(fill = df$wsp_100)) + 
  scale_fill_gradientn(colors = terrain.colors(20))+
  labs(fill = "wskaźnik zachorowalności na 100 000 osób")

df %>%
  st_set_geometry(NULL) %>%
  select(JPT_NAZWA_,wsp_100)%>%
  arrange(by = desc(wsp_100)) %>%
  top_n(10) 
...
```{r}
#gestosc a zachorowania
ggplot(df,aes(x =`ludność na 1 km2` , y=wsp_100 )) + 
  geom_point(size = 3) + 
  geom_smooth(method = lm) +
  scale_x_log10()+
  scale_y_log10()+
  labs(title = "Korelacja między wspólczynnikeim zachorowań a gestoscia ludności na 1km2", 
       y= "wspólczynnik zachorowań", 
       x = "gęstość na 1 km2")

cor.test(df$wsp_100 , df$`ludność na 1 km2`)
```
...
Największa ilość zachowań wystąpiła w województwie śląskim (7 na 10 pierwszych powiatów z największą zachorowalnością) co może wynikac z pojedyńczych ognisk (kopalnie,dps-y) a nie z ogólnej liczby ludności. Moim zdaniem większy udział ma tutaj rozległość terenów miejskich sląska niż sama gestośc zaludnienia. Na tak nierównomierna ilość zachowań może mieć również przeprowadzanie testów w grupach które były narzażone na działanie wirusa, a nie w całych powaitach. 
Być może należałoby przeprowadzić analize kliku czynników.
