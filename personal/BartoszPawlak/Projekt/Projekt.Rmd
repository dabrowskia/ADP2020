...
ogolny2017 <- ggplot(dane_mieszkan_rynek_wlkp,
  aes(x=as.factor(Nazwa),
  y=as.factor(ogolny2017))) +
...
ogolny2018 <- ggplot(dane_mieszkan_rynek_wlkp,
  aes(x=as.factor(Nazwa),
  y=as.factor(ogolny2018))) +
...
Ogolny %>% melt %>%
  ggplot(aes(as.factor(Nazwa), value, fill=variable))+geom_col(position='dodge') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
powiaty <- st_read("D:/1. Studia/2. Magisterka/4. semestr/2. Analiza Danych Przestrzennych/1. Projekt/1. Gotowy — kopia/Projekt/Powiaty.shp", stringsAsFactors = F)
```
