...
```{r}
pmin <- ggplot() + geom_sf(data=woj.el.stats.geom, aes(fill=el.min))
pmax <- ggplot() + geom_sf(data=woj.el.stats.geom, aes(fill=el.max))
pmean <- ggplot() + geom_sf(data=woj.el.stats.geom, aes(fill=el.mean))
plot_grid(pmin, pmax, pmean)
```

Ustawienie w ggplocie skali kolorów oraz przedziałów wartości dla zmiennych jest dramatem.
